...
class ImportCardmarketPriceGuideJob implements ShouldQueue
{
     ...
     */
    public function __construct(string $csvPath, ?string $asOfDate = null, ?string $runUuid = null)
    {
    ...
    public function handle(CardmarketImporter $importer): void
    {
        ...
        Log::channel($logChannel)->info('Price guide import job started', [
            'run_uuid' => $runUuid,
        ...
        try {
            $result = $importer->importPriceGuide($this->csvPath, $run, $this->asOfDate);
            
            if ($result['success']) {
                $run->markSuccess();
                
                Log::channel($logChannel)->info('Price guide import job completed', [
                    'run_uuid' => $runUuid,
            ...
            Log::channel($logChannel)->error('Price guide import job failed', [
                'run_uuid' => $runUuid,
                'error' => $e->getMessage(),
            ]);

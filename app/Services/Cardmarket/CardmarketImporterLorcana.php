...
use App\Models\CardmarketPriceQuoteLorcana;
...
class CardmarketImporterLorcana
{
    protected string $logChannel;
    protected int $chunkSize;
    protected int $progressInterval;
    protected string $timezone;
    protected int $gameId = 19; // Lorcana game ID in CardMarket
    protected int $categoryId = 1629; // Lorcana Singles category
     ...
     * @param CardmarketImportRunLorcana $run
     ...
     */
    public function importProducts(string $jsonPath, CardmarketImportRunLorcana $run, bool $dryRun = false): array
    {
        Log::channel($this->logChannel)->info('Starting Lorcana products import', [
        ...
        try {
            ...
            foreach ($generator as $row) {
                // Filter only Lorcana products (category 1629)
                if (isset($row['id_category']) && $row['id_category'] == $this->categoryId) {
                    $rowsRead++;
                    $batch[] = $row;
                }
                ...
                if (count($batch) >= $this->chunkSize) {
                    ...
                    if ($chunkNumber % $this->progressInterval === 0) {
                        Log::channel($this->logChannel)->info("Lorcana products progress: {$rowsRead} rows read, {$rowsUpserted} upserted");
            ...
            $message = $dryRun 
                ? "Dry run: Would import {$rowsRead} Lorcana products"
                : "Successfully imported {$rowsUpserted} Lorcana products";
            ...
            Log::channel($this->logChannel)->error('Lorcana products import failed', [
                'run_uuid' => $run->run_uuid,
                'error' => $e->getMessage(),
     ...
     * @return array ['success' => bool, 'rows_read' => int, 'rows_upserted' => int, 'message' => string]
     */
    public function importPrices(string $jsonPath, CardmarketImportRunLorcana $run, ?string $asOfDate = null, bool $dryRun = false): array
    {
        ...
        Log::channel($this->logChannel)->info('Starting Lorcana prices import', [
            'run_uuid' => $run->run_uuid,
            'json_path' => $jsonPath,
        ...
        try {
            ...
            foreach ($generator as $row) {
                ...
                if (count($batch) >= $this->chunkSize) {
                    ...
                    // Progress logging
                    if ($chunkNumber % $this->progressInterval === 0) {
                        Log::channel($this->logChannel)->info("Lorcana prices progress: {$rowsRead} rows read, {$rowsUpserted} upserted");
            ...
            $message = $dryRun 
                ? "Dry run: Would import {$rowsRead} Lorcana price quotes for {$asOfDateString}"
                : "Successfully imported {$rowsUpserted} Lorcana price quotes for {$asOfDateString}";
            ...
            Log::channel($this->logChannel)->error('Lorcana prices import failed', [
                'run_uuid' => $run->run_uuid,
                'error' => $e->getMessage(),
                'trace' => $e->getTraceAsString(),
            ]);
    ...
    protected function upsertProductsBatch(array $batch): int
    {
        ...
        try {
            ...
            DB::table('cardmarket_products_lorcana')->upsert(
                $batch,
            ...
            Log::channel($this->logChannel)->error('Failed to upsert Lorcana products batch', [
                'error' => $e->getMessage(),
    ...
    protected function upsertPricesBatch(array $batch): int
    {
        ...
        try {
            ...
            DB::table('cardmarket_price_quotes_lorcana')->upsert(
                $batch,
            ...
            Log::channel($this->logChannel)->error('Failed to upsert Lorcana prices batch', [
                'error' => $e->getMessage(),
                'batch_size' => count($batch),
            ]);

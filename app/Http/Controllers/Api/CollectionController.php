...
class CollectionController extends Controller
{
     ...
     * 
     * Returns product_id, tcgdex_card_id, and cmapi_card_id arrays for checking ownership
     */
    public function getProductIds(): JsonResponse
    {
        ...
        $collection = UserCollection::where('user_id', Auth::id())
            ->select('product_id', 'tcgdex_card_id', 'cmapi_card_id')
            ->get();
        ...
        $cmapiCardIds = $collection->whereNotNull('cmapi_card_id')->pluck('cmapi_card_id')->toArray();

        return response()->json([
            'product_ids' => $productIds,
            'tcgdex_card_ids' => $tcgdexCardIds,
            'cmapi_card_ids' => $cmapiCardIds
        ]);
    }
}

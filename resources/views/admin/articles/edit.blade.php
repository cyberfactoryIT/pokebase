                ...
                <div class="space-y-6">
                    ...
                    <div>
                        ...
                        <select name="game_id" id="game_id" required class="w-full rounded-md border-gray-300 @error('game_id') border-red-500 @enderror">
                            ...
                            @foreach($games as $game)
                                <option value="{{ $game->id }}" {{ old('game_id', $article->game_id) == $game->id ? 'selected' : '' }}>
                                    {{ $game->name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <select name="original_locale" id="original_locale" required class="w-full rounded-md border-gray-300 @error('original_locale') border-red-500 @enderror">
                            <option value="en" {{ old('original_locale', $article->original_locale) == 'en' ? 'selected' : '' }}>English (EN)</option>
                            <option value="it" {{ old('original_locale', $article->original_locale) == 'it' ? 'selected' : '' }}>Italian (IT)</option>
                            <option value="da" {{ old('original_locale', $article->original_locale) == 'da' ? 'selected' : '' }}>Danish (DA)</option>
                        </select>
                        <p class="text-sm text-gray-500 mt-1">Language this article is written in</p>
                    ...
                    <div>
                        ...
                        <textarea name="body" id="body" rows="12" required 
                            class="w-full rounded-md border-gray-300 font-mono text-sm @error('body') border-red-500 @enderror">{{ old('body', $article->body) }}</textarea>
                        @error('body')
                            <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <div class="flex items-center gap-4">
                            <img src="{{ asset($article->image_path) }}" alt="Current image" class="h-20 w-auto rounded border">
                            <div class="flex items-center">
                                <input type="checkbox" name="remove_image" id="remove_image" value="1" class="rounded border-gray-300">
                                <label for="remove_image" class="ml-2 text-sm text-red-600">Remove image</label>
                            </div>
                        </div>
                    </div>
                    @endif
                    ...
                    <div class="flex gap-3 pt-4 border-t">
                        <x-button type="submit" variant="primary">Update Article</x-button>

...
<div class="mb-8">
    <div class="flex items-center justify-end mb-3">
        <a href="{{ route('tcg.expansions.index') }}" class="text-sm text-gray-400 hover:text-white transition flex items-center gap-1 group">
    ...
    <div class="relative group">
        ...
        <div id="expansion-carousel" class="flex gap-4 overflow-x-auto pb-4 scroll-smooth scrollbar-hide">
            @foreach($featuredExpansions as $expansion)
                <a href="{{ route('tcg.expansions.show', $expansion->group_id) }}" 
                   class="flex-shrink-0 w-48 bg-white/5 hover:bg-white/10 border border-white/10 hover:border-white/20 rounded-lg overflow-hidden transition group/card">
                        ...
                        @else
                            <div class="text-center">
                                @if($expansion->abbreviation)
                                    <div class="text-4xl font-bold text-white/40 mb-2">{{ $expansion->abbreviation }}</div>
                                @else
                    ...
                    <div class="p-3 border-t border-white/10">
                        <h4 class="text-white font-medium text-sm mb-1 truncate">{{ $expansion->name }}</h4>
                        @if($expansion->published_on)
                            <p class="text-gray-400 text-xs">{{ $expansion->published_on->format('Y') }}</p>
                        @endif
                    </div>
                </a>
            @endforeach

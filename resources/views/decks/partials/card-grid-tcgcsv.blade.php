...
<div class="grid grid-cols-2 md:grid-cols-4 gap-4">
    @foreach($deck->deckCards->where('product_id', '!=', null) as $deckCard)
    @php
        $card = $deckCard->card;
        if (!$card) continue;
        
        $inCollection = auth()->user()->collection()->where('product_id', $card->product_id)->exists();
    ...
    <div class="deck-card-item bg-white/5 hover:bg-white/10 border border-white/10 rounded-lg transition overflow-hidden group relative" 
        ...
        <div class="absolute top-2 right-2 z-10">
            <button type="button" 
                onclick="event.stopPropagation(); quickAddToCollection({{ $card->product_id }}, null, null, '{{ addslashes($card->name) }}')"

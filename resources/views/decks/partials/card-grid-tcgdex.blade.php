...
<div class="grid grid-cols-2 md:grid-cols-4 gap-4">
    @foreach($deck->deckCards->where('tcgdex_card_id', '!=', null) as $deckCard)
    @php
        $card = $deckCard->tcgdexCard;
        if (!$card) continue;
        
        $inCollection = auth()->user()->collection()->where('tcgdex_card_id', $card->id)->exists();
        // Use /low.webp for grid performance
        $displayImage = ($card->image_large_url ?? $card->image_small_url) . '/low.webp';
        
        // Extract English name from JSON
        $cardName = $card->name;
        if (is_array($cardName)) {
            $cardName = $cardName['en'] ?? $cardName['fr'] ?? $cardName['de'] ?? 'Unknown';
        }
        
        // Extract set name from JSON
        $setNameData = $card->set->name ?? [];
        $setName = is_array($setNameData) ? ($setNameData['en'] ?? 'Unknown Set') : $setNameData;
    @endphp
    ...
    <div class="deck-card-item bg-white/5 hover:bg-white/10 border border-white/10 rounded-lg transition overflow-hidden group relative" 
        ...
        <div class="absolute top-2 right-2 z-10">
            <form method="POST" action="{{ route('collection.add') }}" class="inline" onsubmit="event.preventDefault(); quickAddCardToCollection(null, {{ $card->id }}, '{{ addslashes($cardName) }}', this);">
                @csrf
        ...
        <!-- Card Image -->
        <div class="aspect-[245/342] bg-black/50 overflow-hidden cursor-pointer" onclick="window.location.href='/pokemon/cards/{{ $card->tcgdex_id }}'">
            @if($displayImage)
            <img src="{{ $displayImage }}" alt="{{ $cardName }}" class="w-full h-full object-cover group-hover:scale-105 transition">
            @else
        ...
        <!-- Card Info -->
        <div class="p-3">
            <h4 class="text-white font-semibold text-sm truncate group-hover:text-blue-400 transition cursor-pointer" onclick="window.location.href='/pokemon/cards/{{ $card->tcgdex_id }}'">
                {{ $cardName }}
            </h4>
            ...
            @can('seePrices')
            <!-- Price Display from cache -->
            @if($deckCard->cached_price && $deckCard->cached_price > 0)
            <div class="mt-2">
                @if($preferredCurrency && $deckCard->cached_price_currency !== $preferredCurrency)
                    @php
                        $convertedPrice = \App\Services\CurrencyService::convert($deckCard->cached_price, $deckCard->cached_price_currency, $preferredCurrency);
                    ...
                    <p class="text-gray-500 text-xs">
                        ({{ $deckCard->cached_price_currency === 'EUR' ? '€' : '$' }}{{ number_format($deckCard->cached_price * $deckCard->quantity, 2) }})
                    </p>
                @else
                    <p class="text-green-400 text-xs font-semibold">
            ...
            @else
            <div class="mt-2">
                <p class="text-gray-500 text-xs">{{ __('decks/show.price_unavailable') }}</p>
            </div>
            @endif
            @endcan

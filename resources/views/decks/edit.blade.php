    ...
    <div class="max-w-3xl mx-auto px-6">
        <div class="mb-6">
            <a href="{{ route('decks.index') }}" class="text-gray-400 hover:text-white transition flex items-center gap-2">
                ...
                {{ __('decks/edit.back_to_decks') }}
            </a>
        </div>

        <div class="bg-[#161615] border border-white/15 rounded-2xl shadow-xl p-8">
            <h1 class="text-3xl font-bold text-white mb-2">{{ __('decks/edit.title') }}</h1>
            <p class="text-gray-400 mb-8">{{ __('decks/edit.subtitle') }}</p>
            ...
            <form method="POST" action="{{ route('decks.update', $deck) }}">
                ...
                <div class="mb-6">
                    <label for="name" class="block text-white font-medium mb-2">
                        {{ __('decks/edit.deck_name') }} <span class="text-red-400">{{ __('decks/edit.required') }}</span>
                    </label>
                    <input 
                        type="text" 
                        id="name" 
                ...
                <div class="mb-6">
                    ...
                    <input 
                        ...
                        class="w-full px-4 py-3 bg-black/50 border border-white/20 rounded-lg text-white placeholder-gray-500 focus:outline-none focus:border-blue-500 transition"
                        placeholder="{{ __('decks/edit.placeholder_format') }}"
                    >
                    @error('format')
                        <p class="text-red-400 text-sm mt-1">{{ $message }}</p>
                    @enderror
            ...
            <div class="mt-8 pt-8 border-t border-white/10">
                <h3 class="text-red-400 font-semibold mb-2">{{ __('decks/edit.danger_zone') }}</h3>
                <p class="text-gray-400 text-sm mb-4">{{ __('decks/edit.danger_warning') }}</p>
                <form method="POST" action="{{ route('decks.destroy', $deck) }}" onsubmit="return confirm('{{ __('decks/edit.confirm_delete') }}');">
                    @csrf
                    @method('DELETE')
                    <button type="submit" class="px-4 py-2 bg-red-600 hover:bg-red-700 text-white rounded-lg transition">
                        {{ __('decks/edit.delete_deck') }}
                    </button>
                </form>
            </div>
        </div>
